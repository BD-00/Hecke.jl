...
function one_unit_quotient_with_ctx(P::Hecke.GenOrdIdl, k::Int)::Tuple{FinGenAbGroup, Generic.MapWithSection, Vector{<:GenOrdElem}}
  if k == 1
    O = P.order
    G = abelian_group(zero_matrix(ZZ, 0, 0))
    iso = map_with_preimage_from_func(x -> O(1), x -> G(), G, O)
    return G, iso, elem_type(O)[]
  elseif k == 2
...
function one_unit_quotient_a_b(P, a, b)
  ...
  #G -> 1+P^a/1+P^b:
  im_func = x -> map_G_a_mod_b(x, P, b, gens)

  #1+P^a/1+P^b -> G
  preim_func = x -> disc_log_a_b(x, P, a, b, G, gens)

  iso = map_with_preimage_from_func(im_func, preim_func, G, P.order)
...
function gens_and_rels_a_b(P, a, b)
  ...
  M = basis_matrix(P^(b-a))
  d = [abs(degree(M[i,i])) for i in 1:n]

  #TODO: alternative to Fx(c) in infinite case?
  gens = [1+Fx(c)*x^j*gen_I*B[i] for i in 1:n for j in 0:d[i]-1 for c in Fp_basis]
...
function map_G_mod_b(x::Union{FinGenAbGroupElem, Nemo.MatrixView{ZZMatrix, ZZRingElem}}, P::GenOrdIdl, b::Int, gens::Vector{<:GenOrdElem})
  ...
  for i in 1:_ngens
    ...
    if e == 0
      ...
      y = mod(y*powermod(gens[i], e, P_pow_b), P_pow_b)
    #elseif e < 0
      #compute inverse with invmod, not necessary so far
    end
  end
  return y
end
...
function disc_log_a_b(x::GenOrdElem, P::GenOrdIdl, a::Int, b::Int, G::FinGenAbGroup, gens::Vector{<:GenOrdElem})
  ...
  g = zeros(ZZ, _ngens)
  idx = 1
  for j = 1:n #iterate over coordinates in Fq[x]
    y_j = y_coord[j]
    @assert degree(y_j) < d[j] #test
    for s in 0:d[j]-1 #iterate over powers of x
...
#TODO: fix degree mistake and iterate over diagonal and 1:n
